...
function FlowCanvas({ src, alt }: { src: string; alt: string }) {
  return (
    <div className="relative overflow-hidden rounded-xl border border-white/10 bg-black/40">
      <div className="flex items-center gap-2 border-b border-white/10 bg-white/[0.03] px-4 py-2">
        ...
        <span className="font-mono text-[11px] text-zinc-400">
          n8n workflow · diagrama de referencia
        </span>
      </div>
      <Image
        src={src}
  ...
  return (
    <Section id="automations">
      <SectionHeading
        ...
        subtitle="Agentes y automatizaciones en producción con n8n, RAG y orquestación de LLMs."
      />
      ...
      <div className="flex flex-col gap-6">
        {automations.projects.map((project) => (
          <article
            key={project.title}
            className="group overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/30 p-5 transition-all duration-300 hover:border-white/20 sm:p-7"
          >
            ...
            {/* n8n flow diagram */}
            <div className="mt-6">
              <FlowCanvas src={project.flow} alt={`Flujo n8n — ${project.title}`} />
            </div>

            {/* Live proof for the RAG chatbot */}
            {project.live && "mockup" in project && project.mockup ? (
              <div className="mt-6">
                <div className="mb-4 overflow-hidden rounded-xl border border-white/10 bg-black/40">
                  <div className="flex items-center gap-2 border-b border-white/10 bg-white/[0.03] px-4 py-2">
                    ...
                    <span className="font-mono text-[11px] text-zinc-400">
                      Asistente IA desplegado en el catálogo — vista de referencia
                    </span>
                  </div>
                  <Image
                    src={project.mockup}
                    alt="Chatbot IA en vivo sobre el catálogo universitario"
                    width={1180}
                    height={640}
                    unoptimized
                    className="h-auto w-full"
                ...
                <p className="mb-3 font-mono text-xs uppercase tracking-widest text-accent-soft/80">
                  Desplegado en producción · público y verificable
                </p>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  {automations.deployments.map((dep) => (
                    <Link
                      key={dep.url}
                      href={dep.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group/dep flex items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 transition-all duration-300 hover:border-white/25 hover:bg-white/[0.07]"
                    >
